...
(function ($, learun) {
        ...
        select: {//下拉框
            ...
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {
                    ...
                    dataSource: '0',  // 0数据字典1数据源
                    dataSourceId: '',
                    relation_level: "",//by wxl 级联下拉框
                    relation: "",//by wxl 级联下拉框
                    relation_field: "",//by wxl 级联下拉框
                    itemCode: '',
                    dfvalue: ''       // 默认值
                };
            ...
            property: function ($component) {
                ...
                _html += '<div class="lr-component-control dataSourceId" style="display:none;" ><div id="lr_component_dataSourceId2"></div></div>';
                //start by wxl 级联下拉框
                _html += '<div class="lr-component-title lr_component_relation" style="display:none;">上一级联</div>';
                _html += '<div class="lr-component-control lr_component_relation" style="display:none;" ><div id="lr_component_relation"></div></div>';
                _html += '<div class="lr-component-title lr_component_relation_field" style="display:none;">关联字段</div>';
                _html += '<div class="lr-component-control lr_component_relation_field" style="display:none;" ><div id="lr_component_relation_field"></div></div>';
                //end by wxl 级联下拉框
                _html += '<div class="lr-component-title">默认值<i title="仅在添加数据时默认填入" class="help fa fa-question-circle"></i></div>';
                _html += '<div class="lr-component-control"><div id="lr_component_dfvalue"></div></div>';
                ...
                $parent.find('.lr-compont-item').each(function () {
                    ...
                    if (compontItem.type == 'select' && compontItem.id != dfop.id) {
                ...
                $('#lr_component_relation').lrselect({
                    ...
                    select: function (item, type) {
                        if (!!item) {
                            ...
                            //【上一级联】触发【关联字段】的显示及赋值（如果【关联字段】为空则赋值【显示字段】的值）
                            $('#lr_component_relation_field').lrselectSet(dfop.relation_field || dfop.dataSourceId.split(',')[1]);
                            $('.lr_component_relation_field').show();
                        }
                        else {
                            dfop.relation = '';
                            dfop.relation_field = '';
                            //【上一级联】触发【关联字段】的隐藏及赋值为空
                            $('#lr_component_relation_field').lrselectSet(dfop.relation_field)
                            $('.lr_component_relation_field').hide();
                        }
                    }
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                if (compont.dataSource == '0') {
                    ...
                    var vlist = compont.dataSourceId.split(',');
                    //start by wxl 级联下拉框
                    if (compont.relation != "" && compont.relation_field != "") {
                        $compont.lrselect({
                            select: undefined,
                            // 是否允许搜索
                            allowSearch: true
                        ...
                        function register() {
                            if ($('#' + compont.relation).length > 0) {
                                $('#' + compont.relation).on('change', function () {
                                    ...
                                    var par = {
                                        code: vlist[0],
                                        strWhere: compont.relation_field + ' = \'' + value + '\' ',
                                        queryJson: '{}'
                                    }
                                    learun.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataTable', par, function (data) {
                                        if (data) {
    ...
    function setComponentPropertyHtml($component, verifyData) {
        ...
        _html += '<div class="lr-component-control"><div id="lr_component_proportion"></div></div>';
        //start by wxl 级联下拉框
        _html += '<div class="lr-component-title lr_component_relation_level" style="display:none;">联动级别</div>';
        _html += '<div class="lr-component-control lr_component_relation_level" style="display:none;"><div id="lr_component_relation_level"></div></div>';
        //end by wxl 级联下拉框
        $html.html(_html);
        ...
        $('#lr_component_proportion').lrselectSet(dfop.proportion);
        //start by wxl 级联下拉框
        $('#lr_component_relation_level').lrselect({
            data: [
                {
                    id: '1', text: '1'
                },
            ...
            select: function (item) {
                if (!!item) {
                    dfop.relation_level = item.id;
                }
                else {
                    dfop.relation_level = '';
                }
            }
        }).lrselectSet(dfop.relation_level);
        //end by wxl 级联下拉框
        return $html;
    };
    // 数据字典选择初始化、数据源选择初始化
    function setDatasource(dfop) {
        ...
        //start by wxl 级联下拉框
        $('#lr_component_relation_field').lrselect({// 保存字段
            title: 'text',
            text: 'text',
            value: 'value',
            allowSearch: true,
            select: function (item) {
                if (!!item) {
                    dfop.relation_field = item.value;
                } else {
                    dfop.relation_field = '';
                }
            }
        }).lrselectSet(dfop.relation_field);
        //end by wxl 级联下拉框
            ...
            select: function (item) {
                if (item) {
                    ...
                    learun.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataColName', { code: item.F_Code }, function (data) {
                        ...
                        //start by wxl 级联下拉框
                        $('#lr_component_relation_field').lrselectRefresh({
                            data: fieldData,
                            placeholder: false
                        });
                        //end by wxl 级联下拉框
        ...
        $('#lr_component_dataSource').lrselect({
            ...
            select: function (item) {
                ...
                if (dfop.dataSource == '0') {
                    ...
                    //start by wxl 级联下拉框
                    $('.lr_component_relation').hide();
                    //$('#lr_component_relation_level').lrselectRefresh({
                    //    data: [
                    //        {
                    //            id: '1', text: '1'
                    //        }
                ...
                $('.lr_component_relation_level').show();//by wxl 级联下拉框
            }
